...
This responsible-party has a party-uuid for a non-existent party.</svrl:diagnostic-reference>
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                test="if ( current()/@role-id = ( 'system-owner', 'authorizing-official', 'system-poc-management', 'system-poc-technical', 'system-poc-other', 'information-system-security-officer', 'authorizing-official-poc' ) ) then every $p in oscal:party-uuid satisfies exists(//oscal:party[@uuid eq $p and @type eq 'person']) else true()">
...
This implemented-requirement has an invalid
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                test="every $rrp in $required-response-points satisfies $rrp = $provided-response-points">
